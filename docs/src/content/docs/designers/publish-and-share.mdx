...
<Aside>
  **You'll need a paid plan.** Libraries are only available on Figma's paid plans. On the free
  ...
  the library inherits its name from the source file, so give the file a clear name like "Habitz
  design system".
</Aside>
...
<Aside type="tip">
  To keep a work-in-progress style or component out of the library for good, right-click it in the
  publish dialog and choose **Hide when publishing**. If the **Publish** button is greyed out, it
  usually means there are no publishable changes since last time, or the file has no styles,
...
<Aside type="caution">
  ...
  access token can reach **all** of your Figma files and can't be scoped to a single one, so treat
  it carefully and **revoke** it from the same Security tab if it's ever no longer needed.
</Aside>
...
4. **The source frames or component sets** for component-based tokens, such as `Spacing`, `Radii`,
   `Icons`, or `Assets`.
5. **Publish status** — confirm the latest approved changes have been published.
6. **Rename notes** — list any style, component, frame, or group names that changed since the last
   sync.

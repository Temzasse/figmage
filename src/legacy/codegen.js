...
export class Codegen {
	...
	isGroup(key, value) {
		return (
			isObject(value) && (key === "colors" || (key === "typography" && !("fontSize" in value)))
		);
	}
		...
		for (const [tokenType, tokenValues] of Object.entries(this.tokens)) {
			...
			const tokens = Object.entries(tokenValues)
				...
				.map(([tokenNameOrGroupName, tokenValueOrGroupValues]) => {
					const tokenName = this.formatTokenName(tokenNameOrGroupName, config.tokenCase);
					...
					if (this.isGroup(tokenType, tokenValueOrGroupValues)) {
						tokenValue = Object.entries(tokenValueOrGroupValues)
							.filter((entry) => this.filterByRules(tokenType, entry[0], "token"))
							.reduce((acc, entry) => {
								const tokenNameInGroup = this.formatTokenName(entry[0], config.tokenCase);
			...
			if (config.filetype === "json") {
				...
				fs.writeFileSync(`${outDir}/${filename}.json`, JSON.stringify(json, null, 2));
			}
